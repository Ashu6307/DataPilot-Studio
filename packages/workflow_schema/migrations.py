...
def migrate_workflow_payload(
    payload: dict[str, Any], target_version: str = "1.4"
    ...
    if source_version not in {"1.0", "1.1", "1.2", "1.3", "1.4"}:
        raise WorkflowMigrationError(
            f"WORKFLOW_VERSION_UNSUPPORTED: {source_version}; supported versions are 1.0 through 1.4"
        )
    if target_version not in {"1.1", "1.2", "1.3", "1.4"}:
        raise WorkflowMigrationError(f"WORKFLOW_MIGRATION_TARGET_UNSUPPORTED: {target_version}")
    if source_version == "1.4" and target_version != "1.4":
    ...
    if source_version == "1.0":
        ...
        source_version = "1.1"
    if source_version == "1.1" and target_version in {"1.2", "1.3", "1.4"}:
        ...
        source_version = "1.2"
    if source_version == "1.2" and target_version in {"1.3", "1.4"}:
        ...
        source_version = "1.3"
    if source_version == "1.3" and target_version == "1.4":
        migrated["schema_version"] = "1.4"
        changed.append("schema_version")
        if "dag_workflow_id" not in migrated:
            migrated["dag_workflow_id"] = None
            changed.append("dag_workflow_id")
        if "dag_workflow_version" not in migrated:
            migrated["dag_workflow_version"] = None
            changed.append("dag_workflow_version")
...
def migrate_workflow_file(path: Path, backup_directory: Path) -> WorkflowMigrationReport:
    ...
    temporary.write_text(json.dumps(migrated, indent=2, sort_keys=True, default=str), encoding="utf-8")
