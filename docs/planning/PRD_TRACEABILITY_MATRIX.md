...
| 7 | PRD-XCUT-01 | No fixed rows/columns; metadata execution; immutable inputs; compatibility | Contracts/runtime | M0/M1 | Implemented | `test_vertical_slice.py`, `test_discovery_hardening.py`, `test_background_jobs.py`, `test_migrations.py` | Dynamic discovery, cancellation, checkpoints, and version migration covered |
...
| 10 | FR-10-07 | Detect multiple separated tables | Discovery | M1B | Implemented | `test_discovery_hardening.py` | Row- and column-separated rectangular candidates have stable bounds and IDs |
| 10 | FR-10-08 | Classify schema drift | Schema | M1B | Implemented | `test_schema_drift.py`, Playwright | Explicit findings, policy, confidence, repair, and decision audit |
...
| 11 | FR-11-02 | Calculated/constant canonical fields | Mapping | M1/M1B | Implemented | `test_mapping.py`, `test_expressions.py`, Playwright | Constants/defaults plus versioned typed calculation trees |
...
| 37 | US-001..010 | P0 project/import/discovery/mapping/clean/validate/export/runs/compat/cancel | Vertical slice | M1/M1B | Implemented | API/vertical/background/migration/E2E suites | Complete local P0 journey including cancellation and compatibility migration |
